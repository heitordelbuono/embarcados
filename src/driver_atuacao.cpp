...
void DriverAtuacao::iniciaMotores() {
    ...
    if (e != ESP_OK) { ESP_LOGE(TAG, "i2c_new_master_bus: %s", esp_err_to_name(e)); return; }
    ...
    if (e != ESP_OK) {
        ESP_LOGE(TAG, "PCA9685 nao respondeu (end=0x%02X). Confira SDA/SCL e VCC.", PCA9685_ENDERECO);
        return;
    }
...
void DriverAtuacao::escreveAngulo(int canal, float angulo) {
    ...
    ESP_LOGI(TAG, "canal %d -> %.1f graus (%.0f us, %u ticks)", canal, angulo, us, off);
}
...
void DriverAtuacao::enviaCorrecaoX(float sinal) {
    ...
    escreveAngulo(SERVO_CANAL_X, SERVO_NEUTRO_X - sinal);
#else
    escreveAngulo(SERVO_CANAL_X, SERVO_NEUTRO_X + sinal);
#endif
}
...
void DriverAtuacao::enviaCorrecaoY(float sinal) {
    escreveAngulo(SERVO_CANAL_Y, SERVO_NEUTRO_Y + sinal);
}

void DriverAtuacao::neutro() {
    escreveAngulo(SERVO_CANAL_X, SERVO_NEUTRO_X);
    escreveAngulo(SERVO_CANAL_Y, SERVO_NEUTRO_Y);
}
...
void DriverAtuacao::debug() {
    ...
    printf("-- I2C scan (barramento I2C_NUM_1, SDA=%d SCL=%d) --\n",
           I2C_SDA_GPIO, I2C_SCL_GPIO);
    bool achou = false;
    for (uint8_t addr = 0x08; addr <= 0x77; addr++) {
        esp_err_t r = i2c_master_probe(s_bus, addr, 10);
        if (r == ESP_OK) {
